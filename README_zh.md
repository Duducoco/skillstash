...
<p align="center">
  一个 Hub，所有 Agent。将你的 AI Agent skill 集中管理在一个 git 仓库中，同步到 WorkBuddy、Codex、Claude Code 等。
</p>
...
- **Agent 无关** — 自动检测已安装的 Agent，支持任意组合
- **远端优先初始化** — `init` 要求提供远端 Git URL，确保多设备同步从第一天起就有保障
...
由于 `init` 要求提供远端 URL，多设备同步从一开始就是内置能力：

```bash
# 设备 A（首次）
skillstash init git@github.com:yourname/my-skills.git
# → 创建 Hub，导入本地 skill，推送到远端
...
初始化时会提示选择要管理的 Agent，使用方向键导航，空格切换选中状态，回车确认。之后可通过 `skillstash agents` 修改。

完成 Agent 选择和 skill 导入后，会询问是否立即运行 `link` — 这会将 Hub 中的所有 skill 复制到已管理的 Agent 目录，使其立即可用。

| 远端状态 | 行为 |
|---|---|
| **空仓库** | 本地创建 Hub → 选择 Agent → 自动导入已有 skill → 提示 link → git push |
| **非空且含 `registry.json`** | 克隆 Hub → 选择 Agent → 重新检测本地 Agent → 导入本地新 skill → 提示 link → git push |
